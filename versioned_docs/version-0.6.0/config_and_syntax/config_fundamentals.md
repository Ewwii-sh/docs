...
The root is defined as `tree()` and it is where you should write `defwindow`.
...
Now that you saw this example, you may be wondering why we are doing `tree([])` instead of `tree()`. That is due to another fundamental concept in ewwii which is very important. You will learn about it in the [properties and child definition section](#properties-and-child-definition).
