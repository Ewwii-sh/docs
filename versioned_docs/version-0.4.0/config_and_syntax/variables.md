...
But it is important to note that these variables are locally available only in tree (a.k.a the root) and you need to pass it to other functions with something like `some_fn(foo)` when you want to use a polled variable.
...
Like said above, the `localsignal` does have an additional property which allows modifying the value that it outputs. This property is `mutations`. It can be used like this:
...
fn will_work(time, foo) { // time and foo is passed from `tree([])`
  return box(#{}, [ label(#{ text: time }), label(#{ text: foo }) ]);
}
```
